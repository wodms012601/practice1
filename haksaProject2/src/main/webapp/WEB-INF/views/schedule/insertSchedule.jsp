	...
	<head>
		...
		$(document).ready(function(){
			...
			$("#click").click(function(){
				...
				if("선택" != semesterCode && "선택" != departmentCode && "선택" != gradeCode && "선택" != classCode){
					$.ajax({
						url:'/rest/scheduleSelect'
						, type:'POST'
						, dataType:'JSON'
						...
						, success: function(data){
							console.log("success");
			                $(data).each(function(index,item){
			                	$('#scheduleTable > tbody > tr > td:nth-child(1)').text(item.subjectName);
			                	$('#scheduleTable > tbody > tr > td:nth-child(2)').text(item.humanName);
			                	$('#scheduleTable > tbody > tr > td:nth-child(3)').text(item.buildingName+item.roomFloor+"층"+item.roomName);
